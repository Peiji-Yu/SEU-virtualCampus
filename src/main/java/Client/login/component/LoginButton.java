...
public class LoginButton extends Pane {
    ...
    public LoginButton(){
        background = new Rectangle(250, 40); // 高度从34改为40
        ...
        bp.setCenter(label);
        bp.setMinSize(250, 40); // 高度从34改为40
        getChildren().add(bp);

        addEventHandler(MouseEvent.MOUSE_ENTERED, e -> {
            if(!busy){
                FillTransition ft = new FillTransition(Duration.seconds(0.2), background, Resources.PRIMARY, Resources.SECONDARY);
                ft.play();
        ...
        addEventHandler(MouseEvent.MOUSE_EXITED, e -> {
            if(!busy){
                FillTransition ft = new FillTransition(Duration.seconds(0.2), background, Resources.SECONDARY, Resources.PRIMARY);
                ft.play();
