...
import Client.panel.login.util.InputAnimation;
import Client.panel.login.util.Resources;
...
public class UsernameInput extends AnchorPane {
    ...
    public UsernameInput(String placeholder, TextField tf){
        ...
        textField = tf; textField.setStyle("-fx-background-color:transparent;-fx-border-color:transparent;-fx-font-size:14px;-fx-text-fill:#429866"); textField.setFont(Resources.ROBOTO_REGULAR); textField.setPrefSize(220,28); textField.setFocusTraversable(false); textField.setTranslateX(-10);
    ...
    public void clear(){
        ...
        Client.panel.login.util.InputAnimation.reverse(this, javafx.util.Duration.seconds(0.15));
    }
    // 新增：请求将焦点聚焦到内部文本框
    ...
    public void setText(String text){
        ...
        if (nonEmpty) {
            Client.panel.login.util.InputAnimation.forward(this, javafx.util.Duration.seconds(0.12));
        } else {
            Client.panel.login.util.InputAnimation.reverse(this, javafx.util.Duration.seconds(0.12));
        }
        isChanging = false;
    }
