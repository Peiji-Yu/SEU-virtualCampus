...
public interface ClassStudentMapper {
    // 根据一卡通号查询学生信息
    @Select("SELECT * FROM students WHERE card_number = #{cardNumber}")
    ...
    // 插入新学生
    @Insert("INSERT INTO students (card_number, student_number, major, school, status) " +
    ...
    // 删除学生
    @Delete("DELETE FROM students WHERE card_number = #{cardNumber}")
    ...
    @Select("SELECT MAX(card_number) FROM students")
    Integer getMaxCardNumber();
    
    // 获取最大的学号，用于生成新的学号
    @Select("SELECT MAX(student_number) FROM students")
